...
def all_cov(env_name = 'MountainCar-v0',total_num = 10,called = 'state'):
    import matplotlib.pyplot as plt
    ...
    for i, algorithm in enumerate(num_files):
        ...
        # for num in range(5, 9):
            ...
            cov_dict[num] = [sum(i < score_list[len_3] for i in score_l),
                             sum(i < score_list[len_10] for i in score_l), sum(i < score_list[len_20] for i in score_l),
                             '%.3f' % avg_cover1, '%.3f' % avg_cover2, s_num / 100, s_a_num / 100,
