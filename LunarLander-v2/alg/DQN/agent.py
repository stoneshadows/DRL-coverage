...
# state_grid = create_uniform_grid(env.observation_space.low, env.observation_space.high, bins=(100, 100,100, 100,100, 100,100, 100))
...
class Agent:
    ...
    def train_model(self, env, num_episodes,graph):
        ...
        for i in range(num_episodes):
            ...
            while not done:
                ...
                if type(self.train()) is np.ndarray:
                    for st in self.train():
                        ...
                        # grid_state = discretize(st, state_grid)
                        # action = self.policy(st)
                        #
                        # if '{}'.format(grid_state) in state_dict:
                        #     state_dict['{}'.format(grid_state)] += reward  # 出现过的状态，并记录出现次数
                        # else:
                        #     state_dict['{}'.format(grid_state)] = reward
                        # np.save("state_dict.npy", state_dict)
                        #
                        # if '{},{}'.format(grid_state, action) in state_action_dict:
                        #     state_action_dict['{},{}'.format(grid_state, action)] += reward  # 出现过的状态，并记录出现次数
                        # else:
                        #     state_action_dict['{},{}'.format(grid_state, action)] = reward
                        # np.save("state_action_dict.npy", state_action_dict)
            ...
            # if (avg_reward / log_interval) >= 10:
            #     print("########## Solved! ###########")
                # name = filename + '_solved'
                # policy.save(directory + str(num) + '/')
