...
def all_cov(env_name = 'LunarLander-v2',total_num = 6,called = 'state'):
    ...
    plt.xlabel("Episodes")
    plt.ylabel("Rewards")
    plt.title("LunarLander-v2_{}".format(algorithm))
    ...
    indexs = ('3%({})'.format(round(score_list[len_3], 1)), '10%({})'.format(round(score_list[len_10], 1)),
              '20%({})'.format(round(score_list[len_20], 1)), 'KSCov(%)','SACov(%)',
              'SBCov1','SBCov2','Single','Link','Cross','Mutual')
    g = 10
    ...
    # for num in range(5, 9):
        ...
        cov_dict[num] = [sum(i < score_list[len_3] for i in score_l),
                         ...
                         sum(i < score_list[len_20] for i in score_l),'%.3f' % avg_cover1, '%.3f' % avg_cover2,
