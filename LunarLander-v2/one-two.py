...
def all_cov2(env_name = 'LunarLander-v2',total_num = 10):
    ...
    ###########################################
        ...
        for d in range(4):
            ...
            # X = pd.Series([])
            Y = pd.Series([177.6, 149.96, 188.39, 150.54, 90.78, 227.92])
            # for v in ["pearson", "spearman", "kendall"]:
            for v in ["pearson", "spearman"]:
                print(v, '%.3f' % X.corr(Y, method=v))
                pd.to_numeric(df.iloc[:, d]).tolist().append(X.corr(Y, method=v))
        print(df)
