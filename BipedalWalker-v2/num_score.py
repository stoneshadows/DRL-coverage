...
def all_cov(env_name = 'BipedalWalker-v3',total_num = 10,called = 'state'):
    import matplotlib.pyplot as plt
    ...
    sc = []
    ##################################
    for num in range(0,total_n):
        score_list.extend(np.load("{}{}/scores.npy".format(dir, num), allow_pickle=True).tolist())
        sc.extend(np.load("{}{}/scores.npy".format(dir, num), allow_pickle=True).tolist()[-100:])
    x = [i for i in range(len(score_list))]
    score_list.sort()
    print('avg_rewards:',np.mean(sc))
    ...
    plt.xlabel("Episodes")
    plt.ylabel("Rewards")
    plt.title("BipedalWalker-v3_{}".format(algorithm))
    ...
    indexs = ('3%({})'.format(round(score_list[len_3], 1)), '10%({})'.format(round(score_list[len_10], 1)),
              '20%({})'.format(round(score_list[len_20], 1)), 'SBCov1','SBCov2','KSCov(%)','SACov(%)',
              'Single','Link','Cross','Mutual')
    g = 10
    ...
    # for num in range(5, 9):
        ...
        cov_dict[num] = [sum(i < score_list[len_3] for i in score_l),
                         ...
                         '%.3f' % avg_cover2, s_num/100,s_a_num/100,
